...
export default function EventCard({ event, index }: { event: Event; index: number }) {
  ...
  return (
    <Link to={`/event/${event.id}`} className="block h-full">
      ...
      >
        {/* Inner Card */}
        <div className={`rounded-2xl bg-[#0b0f1a]/90 backdrop-blur-xl p-8 border border-white/10 transition-all duration-300 flex flex-col h-full min-h-[280px]
          ${expired ? 'opacity-70' : 'group-hover:shadow-[0_0_50px_rgba(99,102,241,0.4)]'}
        `}>

          {/* 🔥 Past Badge */}
          {expired && (
            <span className="text-xs px-2 py-1 rounded-full bg-red-500/20 text-red-400 mb-3 w-fit">
              Past Event
            </span>
          )}
          ...
          <div className="mt-auto flex items-center space-x-2 text-indigo-400 font-semibold group/btn">
            <span>{expired ? "View Details" : "View Details"}</span>
